...
module.exports = {
  ...
  deletePosts: async function() {
    ...
    for (const postToDelete of postsToDelete) {
      if (this.rateLimit === -1 || this.rateLimit > 0) {
        console.log(`Deleting: ${postToDelete.post.record.text} (Created ${postToDelete.post.record.createdAt})`)
        try {
          ...
          console.log(`Deleted: ${postToDelete.post.record.text} (Created ${postToDelete.post.record.createdAt})`)
        } catch (error) {
          console.log(`Could not delete: ${postToDelete.post.record.text} (Created ${postToDelete.post.record.createdAt}). Reason: ${error}`)
        }
